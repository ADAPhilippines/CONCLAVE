...
const getConclaveFirstWithThreshold = (
    ...
    return (
        utxosInWallet!.find(
            (e) =>
                e.asset.find((a) => a.unit == policyId) &&
                parseInt(e.asset.find((a) => a.unit == policyId)!.quantity) >= minimumConclave
        ) ??
        utxosInWallet!.find(
            (e) =>
                e.asset.find((a) => a.unit == policyId) &&
                parseInt(e.asset.find((a) => a.unit == policyId)!.quantity) >= 0
        ) ??
    ...
    return (
        ...
        utxosInWallet!.find(
            (e) =>
                e.asset.length === 1 &&
                e.asset.find((a) => a.unit != policyId) &&
                parseInt(e.asset.find((a) => a.unit == 'lovelace')!.quantity) >=
                    parseInt((minimumLovelace / 2).toFixed())
        ) ??
        utxosInWallet!.find(
            (e) =>
                e.asset.length === 1 &&
...
function addSmallConclaveUTXOAvailableWithThreshold(
    ...
    let smallUTXO =
        utxosInWallet!.find(
            (e) =>
                e.asset.find((a) => a.unit == policyId) &&
                parseInt(e.asset.find((a) => a.unit == policyId)!.quantity) < 1_000_000
        ) ??
        utxosInWallet!.find(
            (e) =>
                e.asset.find((a) => a.unit == policyId) &&
                parseInt(e.asset.find((a) => a.unit == policyId)!.quantity) < thresHold
        );
    if (smallUTXO) {
...
function addSmallLovelaceUTXOAvailableWithThreshold(
    ...
    let smallUTXO = utxosInWallet!.find(
        (e) => parseInt(e.asset.find((a) => a.unit == 'lovelace')!.quantity) <= thresHold
    );
...
function addConclaveAvailableWithThreshold(
    ...
    let conclaveUTXO = getConclaveFirstWithThreshold(utxosInWallet, minimumLovelace, minimumConclave, policyId);
    if (conclaveUTXO) {
...
function addLovelaceAvailableWithThreshold(
    ...
    let lovelaceUTXO = getLovelaceFirstWithThreshold(utxosInWallet, minimumLovelace, minimumConclave, policyId);
    if (lovelaceUTXO) {
...
export const generateWorkerBatchesWithThreshold = async (
    ...
    outputBatches!.forEach((element) => {
        ...
        if (!isZero(conclaveOutputSum(element))) {
            addConclaveAvailableWithThreshold(
                utxosInWallet!,
                inputsBatch,
                MinLovelaceAmountPerBatch,
                MinConclaveAmountPerBatch,
                policyId
            );

            while (
                conclaveInputSum(inputsBatch, policyId) < 10_000_000 &&
                conclaveInputSum(utxosInWallet, policyId) > 0
            ) {
                addConclaveAvailableWithThreshold(
                    utxosInWallet!,
                    inputsBatch,
            ...
            while (lovelaceInputSum(inputsBatch) < MinLovelaceAmountPerBatch && lovelaceInputSum(utxosInWallet!) > 0) {
                addAdditionalLovelaceUTXOAvailableWithThreshold(
                    utxosInWallet!,
                    inputsBatch,
                    MinLovelaceAmountPerBatch,
                    policyId
            ...
            addSmallConclaveUTXOAvailableWithThreshold(
                utxosInWallet!,
                inputsBatch,
                MinConclaveAmountPerBatch,
                policyId
            ...
            addLovelaceAvailableWithThreshold(
                utxosInWallet!,
                inputsBatch,
                MinLovelaceAmountPerBatch,
                MinConclaveAmountPerBatch,
    ...
    for (let inProgressPendingReward of inProgressPendingRewards!) {
        let workerBatch: AirdropBatch | undefined = inputOutputBatch.find(
            (i) => i.txHash === inProgressPendingReward.rewards[0].TransactionHash
        );
