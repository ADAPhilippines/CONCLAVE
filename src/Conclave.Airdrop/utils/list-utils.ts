...
export const sortInputDescending = (array: Array<TxBodyInput>, unit: string = 'lovelace'): Array<TxBodyInput> => {
	let _array = array.filter(
		f => (unit == 'lovelace' ? f.asset.length == 1 : f.asset.length != 1) && f.asset.find(f => f.unit == unit)!.unit == unit
	)!;
...
export const getIdxLargestLovelaceReward = (pendingRewards: Array<PendingReward>): number => {
	var i;
	var pendingReward = pendingRewards.find(pendingReward => pendingReward.rewards.find(e => e.RewardType !== 3) === undefined);
...
export function filterUTXOsInWallet(utxosInWallet: Array<TxBodyInput>, addedUTXO: TxBodyInput) {
	let index = utxosInWallet.indexOf(utxosInWallet.find(e => e.txHash === addedUTXO!.txHash && e.outputIndex === addedUTXO!.outputIndex)!);
...
export function addSmallestConclaveOrSmallestLovelaceReward(
	...
	if (
		isOutputSumLarger(conclaveOutputSum([...currentOutputBatch, minConclaveReward]), conclaveInputSum(currentInputBatch)) &&
