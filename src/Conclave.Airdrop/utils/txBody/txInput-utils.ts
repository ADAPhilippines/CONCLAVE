...
export const setTxInputs = (txBuilder: CardanoWasm.TransactionBuilder, txInputs: Array<TxBodyInput>) => {
    txInputs.forEach((txInput) => {
        ...
        if (txInput.asset.find(e => e.unit == policyStr) && txInput.asset.find(e => e.unit == policyStr)!.quantity != "0") {
...
export const getBatchesPerWorker= async (utxosInWallet: Array<TxBodyInput> | null = null, pendingRewards: Array<PendingReward> | null = null): Promise<Array<WorkerBatch>> => {
    ...
    outputBatches!.forEach(element => {
        ...
        if (!isZero(conclaveOutputSum(element))) {
            ...
            let smallUTXO = utxosInWallet!.find(e => e.asset.find(a => a.unit == policyStr) && parseInt(e.asset.find(a => a.unit == policyStr)!.quantity) < 10000000) ??
                            utxosInWallet!.find(e => parseInt(e.asset.find(a => a.unit == "lovelace")!.quantity) < 248000000);
            if (smallUTXO) {
            ...
            let smallUTXO = utxosInWallet!.find(e => parseInt(e.asset.find(a => a.unit == "lovelace")!.quantity) < 248000000);
            if (smallUTXO) {
                inputsBatch.push(smallUTXO);
                utxosInWallet = utxosInWallet.filter(e => (e.txHash != smallUTXO!.txHash) || (e.outputIndex != smallUTXO!.outputIndex));
            }
        }
        let workerBench: WorkerBatch = {
